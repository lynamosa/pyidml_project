...
from pyidml.dml.line import LineFormat
from pyidml.enum.shapes import MSO_AUTO_SHAPE_TYPE, MSO_SHAPE_TYPE
from pyidml.shapes.base import BaseShape
from pyidml.spec import autoshape_types
from pyidml.text.text import TextFrame
...
class AutoShapeType(object):
    ...
    .. attribute:: autoshape_type_id
       ...
       value in ``pyidml.constants.MSO`` like ``MSO_SHAPE.ROUNDED_RECTANGLE``.
    ...
    def __init__(self, autoshape_type_id):
        """Initialize attributes from constant values in pyidml.spec"""
        ...
        if autoshape_type_id not in autoshape_types:
            raise KeyError(
                "no autoshape type with id '%s' in pyidml.spec.autoshape_types"
                % autoshape_type_id
            )
